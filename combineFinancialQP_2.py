...
class StockPitcher:
    ...
    def load_qp_data(self):
        ...
        # qp_files = self.get_sorted_files(self.qp_folder_path, 'merged')[-300:]
...
"""
    ...
    def rearrange_columns(self, df):
        columns_order = ['Date', 'SECU_CODE', 'ADJ_CLOSE_PRICE', 'MvLiqFree_1', 'MvTotal_1', 'MvLiq_1', 'Amount_1',
                         'HP_1', 'LP_1', 'OP_1', 'sk_2', 'Volume_1', 'NetProfitTtm', 'NetProfit', 'OperRevTtm', 'OperRev', 'NetCashOperTtm',
       'EBIT', 'Monetray', 'TotalAssets', 'TotalEquity', 'TotalLiab','GrossMarginMy', 'TotalProfitTtm', 'TotalCurAssets', 'TotalNonCurLiab',
                         'GOperR','GP','GOperCPS']
...
"""
    def calculate_value_factor(self):
        ...
        df['Market_Cap']= df['MvTotal_1']*df['ADJ_CLOSE_PRICE']
...
"""
    def evaluate_factor_IC(self):
        ...
        for value_factor in self.all_factors:
            ...
            # Calculate daily IC
            daily_ic = df.groupby('Date', group_keys=False).apply(lambda x: x[f'{value_factor}_zscore'].corr(x['stocks_return']))
...
"""
    def factor_combination(self):
        ...
        def even_distribution(df):
            ...
            for factor in self.all_factors:
                df[f'normalized_{factor}'] = df.groupby('Date')[factor].transform(normalize).fillna(0).infer_objects(copy=False)
            ...
            df['even_weight_alpha'] = df.groupby('Date')['even_weight_alpha'].transform(normalize).fillna(0).infer_objects(copy=False)
            return df
        ...
        def ic_weighted(df):
            ...
            for factor in self.all_factors:
                if factor != 'even_weight_alpha':
                    df[f'normalized_{factor}'] = df.groupby('Date')[factor].transform(normalize).fillna(
    ...
    def weight_assignment(self, df, positive_filter=True):
        ...
        # Normalize vector within each Date group, handle std == 0
        mean_vector = df.groupby('Date')['vector'].transform('mean')
        std_vector = df.groupby('Date')['vector'].transform('std')
        df['normalized_vector'] = np.where(std_vector == 0, 0, (df['vector'] - mean_vector) / std_vector)
        """
        make sure in top 10 are all positive and bottom 10 are all negative"""

        def quantile_transform(group):
            group = group.dropna(subset=['normalized_vector'])  # Drop NaNs
            try:
                group['quantile'] = pd.qcut(group['normalized_vector'], q=10, labels=False, duplicates='drop')
            except ValueError:
                group['quantile'] = pd.qcut(add_noise(group['normalized_vector']), q=10, labels=False,
                                            duplicates='drop')
            return group
        ...
        df.loc[top_10_mask, 'long_weight'] = abs(
            df['normalized_vector'] / df[top_10_mask].groupby('Date')['normalized_vector'].transform('sum')).astype(float)
        df.loc[bottom_10_mask, 'short_weight'] = (
                    -1 * abs(df['normalized_vector'] / df[bottom_10_mask].groupby('Date')['normalized_vector'].transform('sum'))).astype(
                float)
    ...
    def construct_and_backtest_alpha(self):
        ...
        for value_factor in self.all_factors:
            ...
            df['vector'] = df.groupby('SECU_CODE')[value_factor].shift(1)  # Shift by one to avoid future values
            ...
            def plot_combined_graphs(aggregated, df, initial_principal, vector):
                ...
                # Save the first plot
                plt.tight_layout()
                plt.savefig(f'{output_path}/raw_{value_factor}_pnl.png')
                plt.close(fig)
                ...
                plt.tight_layout()
                plt.savefig(f'{output_path}/raw_{value_factor}_excess.png')
                plt.close(fig)
            ...
            def grouping_analysis(df):
                ...
                # Save the second plot
                plt.tight_layout()
                plt.savefig(f'{output_path}/raw_{value_factor}_grouping.png')
                plt.close(fig)

            grouping_analysis(df)

            output_file = os.path.join(pnl_dir, f'raw_{value_factor}_results.csv')
...
stock_pitcher = StockPitcher(qp_folder_path, financial_data_folder, output_path)
stock_pitcher.run()
